...
public interface UserRepository extends Neo4jRepository<User, Long> {
    ...
    @Query("CREATE (v:User {name:{name}, emailId:{emailId}})")
    User createUser(@Param("name") String name, @Param("emailId") String emailId);

    //query to create viewer and genre relation
    @Query("MATCH (v:User),(g:Genre) WHERE v.emailId={emailId} and g.genre={genre} CREATE (v)-[r:Interested_In]->(g) RETURN r")
    ...
    @Query("MATCH (v:User),(d:Documentary) WHERE v.emailId={emailId} and d.title={title} CREATE (v)-[r:Watches]->(d) RETURN r")
    ...
    @Query("MATCH (v:User),(m:Movie) WHERE v.emailId={emailId} and m.title={title} CREATE (v)-[r:Watches]->(m) RETURN r")
    ...
    @Query("MATCH (v:User),(t:TvEpisodes) WHERE v.emailId={emailId} and t.title={title} CREATE (v)-[r:Watches]->(t) RETURN r")
    ...
    @Query("MATCH (v:User),(w:WebSeries) WHERE v.emailId={emailId} and w.title={title} CREATE (v)-[r:Watches]->(w) RETURN r")
